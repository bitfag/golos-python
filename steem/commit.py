...
class Commit(object):
    ...
    def create_account(
        ...
        """ Create new account in Steem
            ...
            .. note:: Account creations cost a fee that is defined by
               ...
               **You can partially pay that fee by delegating GESTS.**

               To pay the fee in full in GOLOS, leave
               ``delegation_fee_steem`` set to ``0 GOLOS`` (Default).

               To pay the fee partially in GOLOS, partially with delegated
               GESTS, set ``delegation_fee_steem`` to a value greater than ``1
               GOLOS``. `Required GESTS will be calculated automatically.`

               To pay the fee with maximum amount of delegation, set
               ``delegation_fee_steem`` to ``1 GOLOS``. `Required GESTS will be
        ...
        you lose your password!
            ...
            fee of this amount, and delegate the rest with GESTS (calculated
            automatically). Minimum: 1 GOLOS. If left to 0 (Default), full fee
            is paid without GESTS delegation.

            :param str creator: which account should pay the registration fee
                                (defaults to ``default_account``)

            :param bool create_with_delegation: (Optional) If True, a new-style
            account creation will be used, otherwise use AccountCreate op
        ...
        if delegation_fee_steem:
            ...
            # to use GESTS to pay the fee where the ratio must satisfy 1 GOLOS
            # in fee == 5 GOLOS in delegated GESTS
    ...
    def withdraw_vesting(self, amount, account=None):
        """ Withdraw GESTS from the vesting account.
            ...
            :param float amount: number of GESTS to withdraw over a period of
            13 weeks
    ...
    """
        ...
        op = operations.WithdrawVesting(
            **{
                ...
                "vesting_shares":
                    '{:.{prec}f} {asset}'.format(
                        float(amount), prec=6, asset="GESTS"),
            })
    ...
    """
        ...
        op = operations.TransferToVesting(
            **{
                ...
                "amount":
                    '{:.{prec}f} {asset}'.format(
                        float(amount), prec=3, asset='GOLOS')
            })
    ...
    def convert(self, amount, account=None, request_id=None):
        """ Convert SteemDollars to Steem (takes one week to settle)
            ...
            :param float amount: number of GESTS to withdraw
    ...
    def delegate_vesting_shares(self, to_account, vesting_shares,
        ...
        Args:
            ...
            (delegatee).
            vesting_shares (string): Amount of GESTS to delegate eg. `10000
            GESTS`.
